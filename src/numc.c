...
// numc.c mainly handles possible errors and unpacking the variables to then
// later use the functions from matrix.c. To get a better understanding of the
// Python-C interface function calls that are used in the code below, have a
// look at the spec.

static PyTypeObject Matrix61cType;

/* Below are some helper functions for throwing errors */
static int number_methods_err(const char *op, PyObject *args, Matrix61c *self,
                              Matrix61c *other) {
...
/* Helper function to either create a new Matrix61C object with the given
 * new_mat matrix if op_result is non negative */
static PyObject *op_err(Matrix *new_mat, int op_result) {
  if (op_result < 0) {
    ...
    rv->mat = new_mat;
    rv->shape = PyTuple_Pack(2, PyLong_FromLong(new_mat->rows),
...
static int init_rand(PyObject *self, int rows, int cols, unsigned int seed,
  ...
  ((Matrix61c *)self)->shape =
      PyTuple_Pack(2, PyLong_FromLong(rows), PyLong_FromLong(cols));
  return 0;
}
...
static PyObject *Matrix61c_new(PyTypeObject *type, PyObject *args,
                               PyObject *kwds) {
...
Uses allocate_matrix_ref to return a new matrix from the original as a slice if
we are dealing with multiple dimension matrix
*/
static PyObject *Matrix61c_subscript(Matrix61c *self, PyObject *key) {
  ...
  int ref_failed = allocate_matrix_ref(
      &new_mat, self->mat, index * self->mat->cols, self->mat->cols, 1);
  if (ref_failed == -1) {
    PyErr_SetString(PyExc_ValueError, "Dimensions must be positive");
    return NULL;
  ...
  rv->mat = new_mat;
  rv->shape =
      PyTuple_Pack(2, PyLong_FromLong(new_mat->rows), PyLong_FromLong(1));
...
static int Matrix61c_set_subscript(Matrix61c *self, PyObject *key,
                                   PyObject *v) {
  if (!PyLong_Check(key)) {
    PyErr_SetString(PyExc_TypeError, "Key is not valid");
    return -1;
  ...
  if (cols == 1) {
    ...
    for (int i = 0; i < cols; i++) {
      if (!PyFloat_Check(PyList_GetItem(v, i)) &&
          !PyLong_Check(PyList_GetItem(v, i))) {
        PyErr_SetString(PyExc_TypeError, "Value is not valid");
        return -1;
      }
...
/*
 * Adds two numc.Matrix (Matrix61c) objects together. The first operand is self,
 * and the second operand can be obtained by casting `args`.
 */
...
/*
 * Subtracts the second numc.Matrix (Matrix61c) object from the first one. The
...
/*
 * Multiplies two numc.Matrix (Matrix61c) objects together. The first operand is
...
static PyObject *Matrix61c_multiply(Matrix61c *self, PyObject *args) {
  ...
  int alloc_failed =
      allocate_matrix(&new_mat, self->mat->rows, other->mat->cols);
  if (alloc_failed == -1) {
    PyErr_SetString(PyExc_ValueError, "Dimensions must be positive");
    return NULL;
  } else if (alloc_failed == -2) {
 ...
 * argument `optional`.
 */
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow,
                               PyObject *optional) {
 ...
 * methods defined as above. You might find this link helpful:
...
/* INSTANCE METHODS */
/*
 * Given a numc.Matrix self, parse `args` to (int) row, (int) col, and (double)
 * val. Throw a type error if the number of arguments parsed from args is not 3
 * or if the arguments are of the wrong types. Throw an index error if the
 * parsed row and col are out of range. This function should return None in
 * Python.
 */
 ...
 * (double) val. Throw a type error if the number of arguments parsed from args
 * is not 3 or if the arguments are of the wrong types. Throw an index error if
 * the parsed row and col are out of range. This function should return the
 * value at the `row`th row and `col`th column, which is a Python float.
 */
...
static struct PyModuleDef numcmodule = {PyModuleDef_HEAD_INIT, "numc",
